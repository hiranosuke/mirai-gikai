...
### 市民マークの利用に関する注意
さいたま市民マークは自治体規定により**実利用時に申請が必要**な場合がある。本PoCは**ローカル検証用**として
使用し、本番公開時はさいたま市の利用手続きに従う前提とする。spec/PRに明記して引き継ぐ。
...
- **画面確認**: トップ→定例会一覧→議案詳細→インタビューLP を実際に開き、
  ...
  (c) 免責文言とフッターが表示、(d) `/teireikai/...` で到達できることを確認。UI変更のため
  `/pr-screenshot` でスクリーンショットを添付。

## リスク / 既知の未整合（task3へ）

- 議案進捗バーの「衆議院審議 / 参議院審議」表記、`originating_house` の HR/HC ラベルは本PRで未対応。
- AIチャットのチームみらい党説明プロンプトは未対応（チャットの応答には影響が残る）。
- 市民マークの本番利用には さいたま市への申請が必要な可能性（PoC内ローカル利用に限定）。
...
4. `/simplify` → `/review` 通過後、`poc/saitama-mirai-gikai` をベースに PR 作成、スクリーンショット添付。
